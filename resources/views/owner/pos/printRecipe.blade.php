...
<body class="bg-secondary">
    <div class="wrapper">
        ...
        <div class="invoice-header">
            ...
            <div class="inv-date">
                <h3>Date</h3>
                <h4>{{ date('d/m/Y', strtotime($restaurant->customerOrder->created_at)) }}</h4>
            </div>

            <div class="billing-detail">
                <p>Billing to</p>
                <p>{{ $restaurant->name }}</p>
                <p><span>Contact: </span>{{ $restaurant->user->phone }}</p>
                <p><span>Email: </span>{{ $restaurant->user->email }}</p>
                ...
                <table>
                    ...
                    <tbody>
                        @php 
                            ...
                            $totalQuantity = 0; 
                        @endphp
                        @foreach($customerOrderFood as $index => $food)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ $food->name }}</td>
                            <td>{{ $food->quantity }}</td>
